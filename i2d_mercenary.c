...
static int i2d_mercenary_db_load(i2d_mercenary_db *, i2d_string *);
...
int i2d_mercenary_db_init(i2d_mercenary_db ** result, i2d_string * path) {
    ...
    if(i2d_is_invalid(result) || !path) {
        ...
        if(!object) {
            ...
            if(i2d_mercenary_db_load(object, path)) {
...
static int i2d_mercenary_db_load(i2d_mercenary_db * mercenary_db, i2d_string * path) {
    int status = I2D_OK;
    ...
    if(0 > fd) {
        status = i2d_panic("failed to open mercenary db -- %s", path->string);
    } else {
        if(i2d_buffer_create(&buffer, BUFFER_SIZE_LARGE * 2)) {
            ...
            while(0 < result && !status) {
                if(i2d_by_line(&buffer, i2d_mercenary_db_parse, mercenary_db))
                    status = i2d_panic("failed to parse buffer");
                result = i2d_fd_read(fd, BUFFER_SIZE_LARGE, &buffer);
            }
            if(!status && buffer.offset && i2d_by_line(&buffer, i2d_mercenary_db_parse, mercenary_db))
                status = i2d_panic("failed to parse buffer");
